...
- **HARD FORMAT RULE FOR EXTERNAL FIRECRAWL TOOLS:** For any `firecrawl_scrape` call (or `firecrawl_search` with `scrapeOptions`), output format must be **markdown only**. Use `formats: ["markdown"]` and prefer `onlyMainContent: true`.
- **STRICTLY FORBIDDEN with external firecrawl tools:** `rawHtml`, `html`, `links`, `screenshot`, `summary`, `changeTracking`, `branding`, and `json` formats. Do not request them under any circumstance.
- If a page cannot be extracted adequately with markdown-only Firecrawl output, do not switch to raw formats; continue with other markdown-based sources and explicitly note the gap.
...
Include a short **Contact** subsection listing any direct contact methods found for founders/team (email, Telegram, etc.), with citations.
